# pro-study — 6개 언어 실전 프로젝트 & 필사 학습 플랫폼

프로그래밍 언어를 깊이 있게 체화할 수 있도록 설계된 **60개 심층 프로젝트**(C23, Go, Rust, Python, TypeScript, JavaScript 각 10개), **A Tour of Go 인터랙티브 코드 필사(Tracing) 코스**, 그리고 **6개 언어 문법 및 표준 라이브러리 함수 레퍼런스**를 제공하는 종합 학습 플랫폼입니다. 로컬 웹 환경에서 VS Code와 연동하여 코딩하고 결과를 확인하거나, **E-ink 전자책 태블릿에서 스타일러스 펜으로 코드를 직접 따라 쓰며(필사)** 학습할 수 있습니다.
...
### ① 로컬 학습 웹 사이트 (`site/`)
- Go 언어로 바닥부터 구현된 경량 고성능 웹 서버 (`127.0.0.1:8787`).
- **[VS Code에서 열기]** 버튼으로 작업 디렉터리 자동 생성 및 에디터 연동.
- 웹 화면에서 **[빌드] [실행] [테스트]** 버튼을 통해 SSE 실시간 출력 및 단위 테스트 통과 여부 확인, 진행률 자동 저장.
- **📖 다국어 문법 & 표준 함수 레퍼런스 (`/ref`)**: C23, Go, Rust, Python, TypeScript, JavaScript 6개 언어의 핵심 문법과 122개 표준 함수 사전 및 실시간 검색.
- **✍️ A Tour of Go 실전 필사 (`/trace/go`)**: 공식 Tour of Go 19개 핵심 레슨을 한 줄씩 타이핑하며 문법 구조를 완벽 체화.
- **📐 6개 언어 초·중·고 & 대학 CS 수학 필사 (`/trace/math`)**: 한국 교육과정 기반 7대 Stage 40개 모듈의 수식 유도와 6개 언어(C23, Go, Rust, Python, TS, JS) 알고리즘 2중 필사 및 마스터리 진단 평가.
- **💡 아이디어 제안소 (`/ideas`)**: 새로운 프로젝트, 언어, 개선 아이디어를 제안하고 추천(투표)하는 오픈 보드.
- 상세 안내: [`docs/tour-tracing.md`](docs/tour-tracing.md)
- 실행 방법:
  ```powershell
