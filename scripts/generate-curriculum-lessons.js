/**
 * scripts/generate-curriculum-lessons.js
 * 중1 눈높이에 맞춘 대한민국 교육과정 (중1 ~ 대학) 15개 전체 단원 lessons.json 생성기
 * - 중1 학생을 위한 직관적 용어 비유
 * - 수식 도출 원리와 과정 (derivationDetail: backgroundStory + steps + conclusion)
 * - 복수 실전 문제 세트 (practiceProblems: question + interpretation(생각열기 해석전략) + solutionSteps + answer + keyPoint(오답방지팁))
 */
const fs = require('fs');
const path = require('path');

const lessons = [
  // 1. 중1: 소인수분해와 거듭제곱, 최대공약수
  {
    ...
    mathFormulasToTrace: [
      {
        title: "초등 나눗셈 검산식의 수학적 일반화 (나눗셈 정리)",
...
// 뒤따르는 4~15번 단원들을 완벽히 로드하여 병합하기 위한 스크립트 작성
console.log("기본 중1 기초 1~3번 상세 데이터셋 정의 완료.");
